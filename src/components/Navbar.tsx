...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      {mobileMenuOpen && (
        <div className={`absolute top-full left-0 w-full  flex flex-col items-center space-y-4 py-6 md:hidden  transition-all ${
        scrolled ? 'bg-white shadow-md text-gray-800  ' : 'bg-white text-white'
      }`}>
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="text-lg text-gray-800 hover:text-purple-800"
